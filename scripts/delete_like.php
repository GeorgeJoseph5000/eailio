<?php
    ...
    if($iflikemade['user_like'] != ''){
        $queryins = mysql_query("DELETE FROM `likes` WHERE post_id = '$postid' AND user_like ='$un'");
        $seePostId = mysql_fetch_assoc(mysql_query("SELECT * FROM posts WHERE id='$postid'"));
        $likes = $seePostId['likes'];
        $likes--;
        echo '
        <a href="#" onclick="javascript: send_like('.$postid.')"><span class="glyphicon glyphicon-thumbs-up"></span> Like</a>
