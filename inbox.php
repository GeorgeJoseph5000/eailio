...
<hr/>
  ...
  $id=1;
  	$message = '';
  	$query = mysql_query("SELECT * FROM messages WHERE user_to='$un' AND opened='no' AND trash='no'");
  	if(mysql_num_rows($query) == 0){
  		$message = '<span class="error">You do not have unread messages.</span>';
  	}else{
...
<br/>
 ...
 <hr/>
  ...
  $id=1;
  	$message2 = '';
  	$query = mysql_query("SELECT * FROM messages WHERE user_to='$un' AND opened='yes' AND trash='no'");
  	if(mysql_num_rows($query) == 0){
